...
/* ════════════════════════════════════════════════
   ...
   ██████╔╝██║  ██║   ██║   ██║  ██║██████╔╝██║  ██║███████║███████╗
   ╚═════╝ ╚═╝  ╚═╝   ╚═╝   ╚═╝  ╚═╝╚═════╝ ╚═╝  ╚═╝╚══════╝╚══════╝
   IndexedDB — Persistent browser database, survives page refresh
   ════════════════════════════════════════════════ */
...
const AuthService = {
  ...
  async register(data) {
    ...
    await NotificationService.create(user.id,
      role === 'donor'
        ? '🎉 Welcome to REDDROP! Your account is pending admin verification. You\'ll be notified once approved.'
        : '🎉 Welcome to REDDROP! You can now submit blood requests.',
      'welcome');
...
/* ════════════════════════════════════════════════
   BACKEND SERVICE LAYER — Users / Donors
...
const UserService = {
  ...
  async verifyUser(targetId, currentUser) {
    ...
    await dbPut('users', updated);
    await NotificationService.create(targetId, '✅ Your account has been verified by admin! You are now eligible to donate.', 'verification');
...
/* ════════════════════════════════════════════════
   BACKEND SERVICE LAYER — Blood Requests
...
const RequestService = {
  async create(data, currentUser) {
    ...
    for (const donor of matched) {
      await NotificationService.create(donor.id,
        `🚨 ${urgency === 'high' ? 'URGENT: ' : ''}${bloodGroup} blood needed at ${hospital}${city ? ', ' + city : ''}. You are a compatible donor!`,
  ...
  async updateStatus(requestId, newStatus, currentUser, extra = {}) {
    ...
    if (newStatus === 'accepted') {
      await NotificationService.create(request.receiverId,
        `✅ ${currentUser.name} has accepted your ${request.bloodGroup} blood request at ${request.hospital}!`,
    ...
    if (newStatus === 'completed') {
      await NotificationService.create(request.acceptedBy || '',
        `🙏 Your donation for the ${request.bloodGroup} request at ${request.hospital} has been marked complete.`,
        'donation_complete', requestId);
    }
    if (newStatus === 'expired') {
      await NotificationService.create(request.receiverId,
        `⏰ Your ${request.bloodGroup} blood request at ${request.hospital} has expired (72 hours). Please submit a new request if still needed.`,
  ...
  /* Auto-expiry: runs on app load — expire requests older than 72 hours */
  async runExpiryJob() {
...
/* ════════════════════════════════════════════════
   BACKEND SERVICE LAYER — Donations
...
const DonationService = {
  async record(data, currentUser) {
    ...
    await NotificationService.create(donorUser.id,
      `✅ Donation at ${hospital || 'hospital'} recorded. 90-day cooldown starts now. Next eligible: ${new Date(Date.now() + 90*24*60*60*1000).toLocaleDateString()}.`,
...
/* ════════════════════════════════════════════════
   BACKEND SERVICE LAYER — Notifications
...
function navHTML() {
  ...
  return `<nav class="nav">
    ...
    <div class="nav-actions">
      ${u ? `
        ${u.role !== 'admin' ? `<button class="btn btn-ghost btn-sm" onclick="navigateTo('search')">🔍 Find Donors</button>` : ''}
...
function homeHTML() {
  ...
  <div class="hero">
    ...
    <p class="hero-sub">REDDROP bridges the gap between voluntary donors and emergency receivers — fast, intelligent, life-saving.</p>
    <div class="hero-actions">
      <button class="btn btn-primary" style="padding:13px 32px;font-size:16px;border-radius:10px" onclick="openModal('register')">🩸 Become a Donor</button>
      <button class="btn btn-ghost" style="padding:13px 32px;font-size:16px;border-radius:10px" onclick="openModal('request_blood')">💉 Request Blood</button>
    </div>
  ...
  <div style="max-width:1100px;margin:0 auto;padding:0 2rem 4rem">
    <div class="features-grid">
      ${[
        ['🩸','Blood Compatibility Matching','Smart algorithm finds compatible donors based on blood type, location & 90-day eligibility'],
        ['🔔','Instant Notifications','Eligible donors in your area are notified the moment a matching request is submitted'],
        ['🛡️','Admin Verified Donors','Every donor is verified by our admin team before appearing in search results'],
        ['📊','Full Lifecycle Tracking','Request states: Pending → Accepted → Completed, with audit trail and history'],
      ].map(([ic,t,d]) => `
    ...
    <div class="card" style="padding:2.5rem;text-align:center">
      ...
      <p class="text-muted text-sm" style="margin-bottom:2rem">Who can donate to whom? (Donor → Receiver)</p>
      <div style="overflow-x:auto">
        <table class="compat" style="margin:0 auto">
          <tr><th style="background:var(--surface2)">Donor ↓ / Receiver →</th>${BLOOD_TYPES.map(t=>`<th>${t}</th>`).join('')}</tr>
          ${BLOOD_TYPES.map(donor => `
            <tr>
              <td style="font-weight:600;background:var(--surface2)">${donor}</td>
              ${BLOOD_TYPES.map(recv => `<td class="${BLOOD_COMPAT[donor].includes(recv)?'compat-yes':'compat-no'}">${BLOOD_COMPAT[donor].includes(recv)?'✓':''}</td>`).join('')}
            </tr>
          `).join('')}
        </table>
      </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-top:1.5rem">
      <div class="card" style="background:linear-gradient(135deg,#1a0505,#200808);border-color:#3a1010;padding:2rem">
        <h3 style="font-size:20px;margin-bottom:0.5rem">🩸 Are you a Donor?</h3>
        ...
        <button class="btn btn-primary" onclick="openModal('register')">Register as Donor →</button>
      </div>
      <div class="card" style="background:linear-gradient(135deg,#051020,#071828);border-color:#103050;padding:2rem">
        <h3 style="font-size:20px;margin-bottom:0.5rem">💉 Need Blood?</h3>
        ...
        <button class="btn btn-ghost" style="border-color:var(--blue);color:var(--blue-text)" onclick="openModal('request_blood')">Request Blood Now →</button>
      </div>
    </div>
  </div>`;
}
    ...
    donors.map(d => `
      <div class="donor-card" style="margin-bottom:10px">
        ...
        <div class="donor-info">
          ...
          <div class="donor-meta">📍 ${d.city || 'Unknown'} &nbsp;·&nbsp; ${d.donationCount || 0} donation${(d.donationCount||0)!==1?'s':''} &nbsp;·&nbsp; Joined ${new Date(d.joinDate||d.createdAt).toLocaleDateString()}</div>
          ${canDonate(d) ? `<div class="text-xs text-muted" style="margin-top:2px">📞 Contact visible after request acceptance</div>` : `<div class="text-xs text-muted" style="margin-top:2px">⏳ In 90-day cooldown · ${Math.max(0,90-daysSince(d.lastDonationDate))}d remaining</div>`}
        </div>
...
function sidebarHTML(u) {
  ...
  const icons = { dashboard:'🏠', profile:'👤', donate:'🩸', 'my-donations':'📋',
    notifications:'🔔', 'my-requests':'📋', users:'👥', requests:'📊', analytics:'📈', audit:'🔐' };
  ...
  return `<div class="sidebar">
    <div class="sidebar-user">
      ...
      <div>
        ...
        <div class="sidebar-role">${u.role === 'admin' ? 'Administrator' : u.bloodGroup ? u.bloodGroup + ' · ' + u.role : u.role}</div>
      </div>
    </div>
    <div class="sidebar-nav">
      ...
      `).join('')}
      <hr class="divider"/>
      <div class="sidebar-item" onclick="doLogout()"><span class="sidebar-icon">🚪</span>Sign Out</div>
    </div>
  </div>`;
}
...
function userDashHTML(u) {
  ...
  return `
  <h2 class="serif" style="font-size:28px;margin-bottom:1.5rem">Welcome back, ${u.name.split(' ')[0]} 👋</h2>
  <div class="stats-grid">
    ${u.role === 'donor' ? `
      <div class="stat-card"><div class="stat-label">Total Donations</div><div class="stat-value stat-red">${u.donationCount || 0}</div></div>
      <div class="stat-card"><div class="stat-label">Days Since Last</div><div class="stat-value stat-amber">${days === 9999 ? '—' : days}</div></div>
      <div class="stat-card"><div class="stat-label">Status</div><div class="stat-value" style="font-size:16px">${eligible ? '✅ Eligible' : '⏳ Cooling'}</div></div>
    ` : `
    ...
    <div class="card">
      ...
      <div style="margin-top:1.25rem;display:flex;gap:8px;flex-wrap:wrap">
        ...
        <button class="btn btn-ghost btn-sm" onclick="doToggleAvailability()">
          ${u.availability ? '⏸ Mark Unavailable' : '▶ Mark Available'}
        </button>
        <button class="btn btn-ghost btn-sm" onclick="navigateTo('search')">🔍 Browse Requests</button>
      </div>
    </div>
  ` : `
    <div class="card">
      ...
      ${myReqs.length === 0 ? '<div class="empty" style="padding:2rem"><div class="empty-icon">💉</div><p>No requests yet.</p></div>' :
...
function requestRowHTML(r) {
  ...
  return `<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)">
    ...
    <div style="flex:1;min-width:0">
      ...
      <div class="text-muted text-xs">${r.city} · ${new Date(r.createdAt).toLocaleDateString()}</div>
    </div>
...
function profileHTML(u) {
  return `<div class="card">
    ...
    <div style="display:flex;align-items:center;gap:1.5rem;margin-bottom:2rem;padding-bottom:1.5rem;border-bottom:1px solid var(--border)">
      ...
      <div>
        ...
        <div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">
          ...
          <span class="badge ${u.verified?'badge-green':'badge-amber'}">${u.verified?'✓ Verified':'⏳ Pending Verification'}</span>
    ...
    <div class="form-group">
      ...
      <select id="p-avail">
        <option value="true" ${u.availability?'selected':''}>✅ Available to Donate</option>
        <option value="false" ${!u.availability?'selected':''}>⏸ Not Available</option>
      </select>
    </div>
    ` : ''}
...
function donateHTML(u) {
  ...
  return `<div class="card">
    ...
    <div class="info-box" style="margin-bottom:1rem">
      ℹ After logging, your availability will be set to <strong>Unavailable</strong> and 90-day restriction starts.
    </div>
    <button class="btn btn-primary" onclick="doLogDonation()">🩸 Confirm & Log Donation</button>
  </div>`;
}
...
function donationHistoryHTML(u) {
  ...
  return `<div class="card">
    ...
    ${donations.length === 0 ? '<div class="empty"><div class="empty-icon">📋</div><p>No donations recorded yet.</p></div>' :
    `<table class="table">
...
function myRequestsHTML(u) {
  ...
  return `<div class="card">
    ...
    ${mine.length === 0 ? '<div class="empty"><div class="empty-icon">💉</div><p>No requests yet. Create your first.</p></div>' :
    mine.map(r => `
      <div class="card card-sm" style="margin-bottom:10px;background:var(--surface2);border:1px solid var(--border)">
        <div style="display:flex;align-items:flex-start;gap:12px">
          ...
          <div style="flex:1">
            ...
            <div class="text-muted text-sm">${r.city} · ${new Date(r.createdAt).toLocaleDateString()} · ${r.urgency} urgency</div>
            ...
            ${r.acceptedByName ? `<div class="text-sm" style="margin-top:6px;color:var(--green-text)">✓ Accepted by: <strong>${r.acceptedByName}</strong></div>` : ''}
          </div>
...
function notificationsHTML(u) {
  ...
  return `<div class="card">
    <div class="card-header">
      ...
      <button class="btn btn-ghost btn-sm" onclick="doMarkAllRead()">✓ Mark all read</button>
    </div>
    ...
    notifs.map(n => `
      <div class="notif-item" onclick="markNotifRead('${n.id}')">
        ...
        ${!n.read ? `<button class="btn btn-ghost btn-sm" onclick="event.stopPropagation();markNotifRead('${n.id}')">✓</button>` : ''}
      </div>
    `).join('')}
  </div>`;
}
...
function adminDashHTML() {
  ...
  ${pendingUsers.length > 0 ? `
    <div class="card" style="margin-bottom:1rem;border-color:#5a3000;background:#100a00">
      <div class="card-header"><span class="card-title">⚠️ Pending Verifications (${pendingUsers.length})</span></div>
      ${pendingUsers.map(u => `
        <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)">
          ...
          <div style="flex:1">
            ...
            <div class="text-xs text-muted">${u.email} · ${u.bloodGroup} · ${u.city}</div>
          </div>
  ...
  return `<div class="card">
    ...
    <table class="table">
      ...
      <tbody>${users.map(u => `<tr>
        ...
        <td>
          ${u.role==='donor'?`<span class="badge ${u.verified?'badge-green':'badge-amber'}">${u.verified?'✓ Verified':'⏳ Pending'}</span>`:''}
    ...
    logs.map(l => `
      <div class="audit-row">
        ...
        <div style="flex:1">
          ...
          <div class="text-xs text-muted">by ${l.performedBy} · ${timeAgo(l.createdAt)}</div>
        </div>
...
function modalHTML() {
  ...
  if (m === 'login') {
    inner = `
      ...
      <div class="info-box" style="margin-bottom:1.25rem">
        Demo accounts:<br/>
        <strong style="color:var(--text)">arif@demo.com</strong> / <strong style="color:var(--text)">demo</strong> (donor) &nbsp;·&nbsp;
        <strong style="color:var(--text)">sumaiya@demo.com</strong> / <strong style="color:var(--text)">demo</strong> (receiver) &nbsp;·&nbsp;
      ...
      <div style="text-align:center;margin-top:1rem;font-size:13px;color:var(--text3)">No account? <span style="color:var(--red);cursor:pointer" onclick="openModal('register')">Register here →</span></div>`;
  }

  else if (m === 'register') {
    inner = `
      <div class="modal-title">Join REDDROP 🩸</div>
      ...
      <div style="text-align:center;margin-top:1rem;font-size:13px;color:var(--text3)">Have an account? <span style="color:var(--red);cursor:pointer" onclick="openModal('login')">Sign in →</span></div>`;
  }

  else if (m === 'new_request' || m === 'request_blood') {
    const prefill = appState.prefillBlood || '';
    inner = `
      <div class="modal-title">🩸 Request Blood</div>
      <div class="modal-sub">Compatible donors near you will be notified immediately</div>
      ${!appState.user ? `<div class="warning-box" style="margin-bottom:1rem;border-radius:var(--radius-sm)"><span style="color:var(--amber-text)">⚠ Please <span style="cursor:pointer;text-decoration:underline" onclick="openModal('login')">sign in</span> as a receiver to submit a request.</span></div>` : ''}
      ${appState.user && appState.user.role !== 'receiver' ? `<div class="warning-box" style="margin-bottom:1rem;border-radius:var(--radius-sm)"><span style="color:var(--amber-text)">⚠ Only receiver accounts can submit blood requests.</span></div>` : ''}
      <div class="form-row">
  ...
  return `<div class="modal-overlay" onclick="if(event.target===this)closeModal()">
    <div class="modal">${inner}<button class="modal-close" onclick="closeModal()">✕</button></div>
  </div>`;
}

/* ════════════════════════════════════════════════
   ACTION HANDLERS — wiring UI to backend services
   ════════════════════════════════════════════════ */
...
async function doRegister() {
  ...
  appState.sidebarTab = 'dashboard';
  await refreshUserData();
  toast('Account created! Welcome to REDDROP 🎉', 'success');
  render();
}
...
async function doToggleAvailability() {
  ...
  if (res.error) { toast(res.error, 'error'); return; }
  appState.user = res.user;
  toast(`Availability set to: ${res.availability ? 'Available ✅' : 'Unavailable ⏸'}`, 'info');
  render();
}
...
async function doLogDonation() {
  ...
  if (fresh) appState.user = AuthService.sanitize(fresh);
  await refreshUserData();
  toast('Donation logged! Thank you 🙏', 'success');
...
async function doSubmitRequest() {
  ...
  await refreshUserData();
  toast(`Request submitted! ${res.matchedDonors} donor(s) notified 🔔`, 'success');
  render();
}
...
async function doCompleteRequest(id) {
  ...
  if (res.error) { toast(res.error, 'error'); return; }
  await refreshUserData();
  toast('Request marked as completed ✓', 'success');
  render();
}
...
/* ─── Donor accept request (from search or notifications) ─── */
